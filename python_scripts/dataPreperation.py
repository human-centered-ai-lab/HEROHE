    ...
    for root, dirs, files in os.walk("D:\GoogleDrive\Arbeit\HEROHE_Challenge\ExtractedDataV2", topdown=False):
       for file in tqdm(files):
          if file.endswith(".txt"):
              ...
              query_insert_csv = "COPY corrected.image_parameter (filename, name, class, parent, roi, centroid_x_um, centroid_y_um, nucleus_area, nucleus_perimeter, nucleus_circularity, nucleus_max_caliper, " + \
              ...
              pgCursor.execute("INSERT INTO images(name) VALUES ('" + file.replace(".mrxs", "") + "');")
...
def linkAndUpdateDatabase():
    query_updare_link = "UPDATE image_parameter SET image_id=sub.id FROM ( " + \
                        "SELECT id, name fn FROM images WHERE her2status = 'TEST') sub " \
...
def updateParameter():
    ...
    querys_update_parameters.append("UPDATE herohe_data SET mean_nucleus_area=sub.upvalue " + \
            ...
            "FROM corrected.images i " + \
            "JOIN corrected.lerndata_v2 l ON i.id = l.image_id " + \
